...
- **Every control is a finger tall.** The pager's buttons are 48 px (`--control-pager`); every
  ...
  *Got it* (#147), and the shut notice's way on (#163). The index's filled *Continue* is the
  one exception to the pattern, because a fill is painted over its padding: the link is the
  padded target and a span inside it is the button a reader sees, at the size it always had.
  The index's language control is not an exception but a different shape: it is drawn as
  outlined boxes 44 px tall, so its target is the box a reader sees (#163).
