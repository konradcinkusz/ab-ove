...
%% reader's place when they reveal. An anonymous reader's place is held under an opaque
%% cookie, not a token (ADR-0061), and what the reader writes on a frame still stays in
%% their browser (ADR-0039).
...
flowchart TD
  ...
  REVEAL -->|"raises the reader's place"| API
