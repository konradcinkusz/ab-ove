...
%
%  STYLU DOMOWEGO NIE MA W TYM PLIKU I O TO CHODZI. Jest w house-preamble.tex obok,
%  wciagany przez \input nizej, jedna kopia na to repozytorium, przypieta skrotem do
%  docs/research/HOUSE-PREAMBLE.tex w architecture-standards. Obie edycje tego papieru
%  zjechaly z tego stylu calkowicie -- zadnego z jego siedmiu znacznikow -- podczas gdy dwa
%  inne repozytoria trzymaly siedem na siedem, a nic tego nie powiedzialo, bo nic ich nie
%  porownywalo. Zapisuje to ADR-0058, a pilnuje scripts/check-papers.mjs.
%
%  Wszystko, czym ta edycja rozni sie od angielskiej siostry, to dwa \defy przed \input.
%  Nie ma drugiej preambuly, bo dwie preambuly sie rozjezdzaja.
%
...
% ------------------------------------------------------------------------------------
% KONTRAKT PREAMBULY DOMOWEJ. Cztery polecenia, zdefiniowane tutaj, bo house-preamble.tex
% czyta je w trakcie wczytywania; zdefiniowanie ich po \input daje dokument z pustym
% naglowkiem biezacym i zlymi metadanymi PDF-a.
%
% Dwie linie ponizej to cala roznica miedzy ta edycja a angielska: \houselang przelacza
% babel, a \editionsuffix wybiera polskie renderowanie kazdego diagramu. Zadnej drugiej
% preambuly - dwie preambuly sie rozjezdzaja, a rozjazd widac potem jako roznice
% typograficzna, ktorej nikt nie umie przypisac do zmiany.
% ------------------------------------------------------------------------------------
\def\houselang{polish}
\def\editionsuffix{.pl}

\newcommand{\paperstatus}{NIC NIE JEST WDROZONE}
\newcommand{\headerleft}{ab-ovo --- przeglad projektu}
\newcommand{\pdftitleline}{ab-ovo: platforma edukacyjna, ktora zamyka w sobie Matematyke od zera dla inzyniera AI}
...
\housetitle
    {ab-ovo}
    {Platforma edukacyjna, która zamyka w sobie \emph{Matematykę od zera dla inżyniera AI}}
    {konradcinkusz}
    {\url{https://github.com/konradcinkusz/ab-ovo}}
    {źródło prawdy: \texttt{README.md}, \texttt{docs/architecture/00-ARCHITECTURE.md},
     \texttt{docs/adr/}, \texttt{docs/ux/UI-UX.md} --- gdzie ten papier i któryś z nich
     się nie zgadzają, rację ma markdown}
...
ab-ovo jest oprogramowaniem, które ma jak. Czytelnik czyta ramkę, zobowiązuje się do odpowiedzi
i odsłania kolejną ramkę, która otwiera się odpowiedzią. Odsłonięcie jest formularzem, a nie
przełącznikiem: prosi API o przesunięcie pozycji w lekturze, a API podaje kolejną ramkę dopiero
potem, więc odpowiedź na ramkę widoczną na ekranie renderuje żądanie o kolejną i nic wcześniej:
nie ma jej w dokumencie, zamiast być w nim ukrytą, a czytelnik, który otworzy inspektor, nie
znajdzie niczego. Tam, gdzie ramka o coś prosi, pod nią stoi arkusz --- linia odpowiedzi,
notatnik liczący arytmetykę, płótno do szkicu --- a wszystkie trzy są opcjonalne i lokalne dla
przeglądarki czytelnika. Ćwiczenia komputerowe książki robi się w osobnym panelu laboratorium,
w którym Python działa we własnej przeglądarce czytelnika; opuściły one pętlę lektury celowo,
bo czytelnik książki matematycznej nie powinien musieć pisać kodu, żeby odpowiedzieć na ramkę.

\begin{figure}[ht]
\centering
\includegraphics[width=0.78\linewidth]{\dgm{b1-reader-loop}}
\caption{Pętla czytelnika. Każde pole działa bez konta, a każda ramka przychodzi z API,
o które przy każdym żądaniu pyta własny serwer aplikacji webowej; dwie kropkowane krawędzie
wychodzące z pętli są opcjonalne.}
\end{figure}

Pierwsze wymogi produktu ograniczają wszystko inne w tym dokumencie i są dwa:
\textbf{lektura nie potrzebuje konta, a każda ramka przychodzi z API.} Kiedyś były jednym
wymogiem, że pętla nie potrzebuje też serwera; ADR-0060 odwrócił tę połowę, bo czytelnik,
którego ramki przychodzą z serwera, do którego nie może dotrzeć, nie degraduje się łagodnie ---
po prostu przestaje czytać. Ramki podaje \texttt{AbOvo.Api} krok po kroku, bramkowane pozycją
w lekturze --- pozycję anonimowego czytelnika przechowuje nieprzezroczyste ciasteczko, a nie
token --- ćwiczenia działają po stronie klienta, a nic, co czytelnik pisze na ramce, nie jest
...
Ekran usuwania mówi to czytelnikowi wprost, obok tego, co usunięcie faktycznie usuwa.
...
instancji, pod żadnym adresem, dla nikogo. Cztery pliki \texttt{fly.toml} opisują topologię,
...
ramka na ekran, z własną prozą i matematyką książki; pasek nawigacji przypięty do dołu
ekranu z podpisanymi przyciskami \emph{Wstecz} i \emph{Dalej}, a między nimi pozycja, która
otwiera mapę programu; oraz arkusz na każdej ramce, która o coś prosi. Za tym:
...
\midrule
\texttt{src/AbOvo.AppHost}        & Korzeń kompozycji. Tylko deweloperski; nie jest topologią produkcyjną. \\
\texttt{src/AbOvo.ServiceDefaults} & Wspólne jądro: instalacja przekrojowa, pod pułapem rozmiaru. \\
\texttt{src/AbOvo.Contracts}      & DTO przekraczające granicę serwisu. \\
\texttt{src/AbOvo.Api}            & Serwis HTTP. Posiada jedną logiczną bazę. Weryfikuje tokeny; nie wydaje żadnych. \\
\texttt{tests/AbOvo.Api.Tests}    & Jednostkowe i integracyjne w pamięci, plus reguły architektury. Bez kontenera. \\
\texttt{tests/e2e}                & Pakiet akceptacyjny Playwrighta; własny pakiet i własny lockfile. \\
\texttt{web/}                     & Przestrzeń pnpm. Jedna aplikacja Next.js i jej backend-for-frontend. \\
\texttt{flyio/}                   & Wdrożona topologia, opisana i nigdy niezastosowana. \\
\texttt{docs/}                    & Architektura, decyzje, UX, diagramy i ten papier. \\
\bottomrule
\end{tabular}
\end{table}
...
standardów, a \texttt{docs/architecture/00-ARCHITECTURE.md} przechodzi jej piętnaście zasad po
...
Pięć faz, uszeregowanych w \texttt{docs/ux/UI-UX.md} tak, by sesja dostarczająca podnosiła
...
\begin{description}[leftmargin=0pt,itemsep=1pt]
  \item[\texttt{docs/START-HERE.pl.md}] Drzwi frontowe, po polsku i po angielsku: którego z
        czterech rodzajów dokumentu potrzebujesz i gdzie on jest.
  \item[\texttt{README.md}] Czym system jest, jak uruchomić go jednym poleceniem, i antycele,
        wypowiedziane przed funkcjami.
  \item[\texttt{docs/architecture/00-ARCHITECTURE.md}] Przejście zasada po zasadzie, rejestr
        odstępstw i znane luki.
  \item[\texttt{docs/adr/}] Decyzje, po jednym pliku na każdą, wraz z tym, co kosztowały.
  \item[\texttt{docs/DIAGRAMS.pl.md}] Cały system jako diagramy, w czterech częściach, po polsku
        i po angielsku. Każdy istnieje w treści dokumentu --- jedyna forma, którą renderuje
        GitHub --- oraz jako samodzielny \texttt{.mmd}, z którego renderowane są rysunki tego
        papieru.
  \item[\texttt{docs/SCREENSHOTS.pl.md}] Produkt ekran po ekranie, po polsku i po angielsku,
        uchwycony z buildu produkcyjnego przez pakiet akceptacyjny.
  \item[\texttt{docs/tutorials/}, \texttt{docs/how-to/}] Dwujęzyczne w całości: lekcje na pierwsze
        uruchomienie i przepisy na zadanie już zrozumiane.
  \item[\texttt{docs/ux/UI-UX.md}] Ekrany tak, jak zbudowane, i uszeregowany backlog.
  \item[\texttt{AGENTS.md}, \texttt{CONTRIBUTING.md}, \texttt{SECURITY.md}] Praca w repozytorium i
