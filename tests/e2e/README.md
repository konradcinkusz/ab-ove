...
| `edition-pages.spec.ts` | from the Polish index, sign-in, registration and `/about` are Polish, down to the document's own language; a failed sign-in keeps the address without the URL; restarting the second step keeps the destination; a new password's rules are its field's description, checked by the browser (#166) |
