...
function isSameOrigin(request: Request): boolean {
  ...
  // The host as the reader's browser addressed it. Fly terminates TLS and rewrites `host`,
  // so `x-forwarded-host` is consulted first; comparing against `request.url` instead would
  // compare the browser's host with the container's and refuse every request in production.
  const forwarded = request.headers.get('x-forwarded-host');
  const expectedHost = (forwarded ?? request.headers.get('host') ?? '').split(',')[0]?.trim();
