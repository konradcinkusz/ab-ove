...
/**
 * The deletion screen, and only that.
 *
 * ──────────────────────────────────────────────────────────────────────────────────────
 * A PAGE OF ITS OWN, ONE LINK BEYOND THE ACCOUNT'S OVERVIEW (issue #161).
 *
 * It was `/account` itself, so the account link on the index opened a page whose heading
 * was "Delete your account" and which said nothing else about the account. `/account` is
 * now the reader's overview, and this screen moved here unchanged in substance: the same
 * four things said before the button (ADR-0021), the same form posting to the same route,
 * which sends a failure back to this address rather than to the overview. Its way out,
 * *Keep my account*, returns to the overview it was reached from.
 * ──────────────────────────────────────────────────────────────────────────────────────
 *
 * ──────────────────────────────────────────────────────────────────────────────────────
 ...
 * is in and already labels itself in it. The language rides both hrefs, and a reader who
 * arrives without one gets English by `chromeFor`'s ordinary fallback. Nothing is being
 * asserted about interface preference — the same signal that decided the word on the link
 * decides the words on the pages it opens.
 *
