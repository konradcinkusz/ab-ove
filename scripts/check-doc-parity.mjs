#!/usr/bin/env node
/**
 * check-doc-parity.mjs — the bilingual documents come in pairs, and neither half may be
 * edited without the other.
 *
 * WHY THIS EXISTS:
 *
 *   This product is bilingual because the BOOK is: 47 programs set in English and Polish,
 *   with the edition the reader's own choice and no default (ADR-0015). A documentation set
 *   that was English only would be telling a Polish reader that the product is for them and
 *   the explanation of it is not.
 *
 *   Two copies of a document is a drift surface, and the failure mode is specific: somebody
 *   fixes a wrong command in the English guide, the Polish guide goes on telling its reader
 *   to run the wrong thing, and nothing goes red. That is WORSE than having no translation,
 ...
 *
 *   R2 cannot check that a translation is CORRECT; no script can. It checks that somebody
 *   looked. That is the same bargain the content digest in fetch-book-content.sh makes, and
 *   the same reasoning: a convention about remembering is a convention that fails on the busy
 *   week.
 *
 * NOT EVERY DOCUMENT IS IN HERE, ON PURPOSE. The ADRs, the architecture register and the UX
 * plan are English only and stay that way: they are a record of reasoning aimed at whoever
 * maintains the code, they move constantly, and a half-maintained translation of a decision
 * log is a second decision log that disagrees with the first. What is bilingual is what a
 * READER or a first-time contributor reaches first — the front door, the tutorials, the
 * how-to guides, the diagrams and the picture tour.
 *
 * R2 only runs when a base ref is given, because it needs a diff. CI passes origin/main;
 * locally you can pass anything `git diff` understands, or omit it and get R1 alone.
 *
...
/**
 * Where bilingual documents live. A directory listed here is bilingual IN FULL: every `*.md`
 * in it must have a `*.pl.md` twin. Adding a directory here is how you opt a new area in.
 */
const BILINGUAL = ['docs/tutorials', 'docs/how-to'];

/** Individual files that are bilingual without their whole directory being so. */
const BILINGUAL_FILES = ['docs/START-HERE.md', 'docs/DIAGRAMS.md', 'docs/SCREENSHOTS.md'];
...
if (baseRef) {
  ...
  try {
    ...
    // No merge base — a fork with no shared history, or a shallow clone that does not reach
    // it. Structural parity still held, and reporting a coupling failure nobody can act on is
