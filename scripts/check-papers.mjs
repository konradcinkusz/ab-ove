 ...
 *
 * Adopted from architecture-standards, docs/research/CHECK-PAPERS.mjs (see that
 * repository's 00-RESEARCH-DOCUMENTATION.md, "Checking the papers"). Runs in the
 * same lint job as check-links, check-diagrams and check-doc-parity:
 *
 *   npm run lint:papers
 *
 * THIS REPOSITORY IS WHY THE CHECK EXISTS. Both overview editions carried none of
 * the seven house markers while agent-eval-bench and marcus-shop carried all seven,
 * and nothing said so for as long as nobody put two PDFs side by side.
 *
 * WHY THIS EXISTS.
 *
