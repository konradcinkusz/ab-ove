...
#     superuser password is written ONCE, when initdb first creates an empty data directory.
#     AppHost.cs calls WithDataVolume("ab-ovo-pgdata"), so that directory OUTLIVES the run
#     that made it - and step 3 above generating a fresh Parameters:auth-db-password leaves
#     the volume holding the OLD one. Nothing reconciles the two, and the containers that
#     WaitFor a database never get their turn.
#     -> docker volume rm ab-ovo-pgdata     (local development data only: the API migrates
#        its schema on startup and authservice uses EnsureCreated, so both databases rebuild
#        themselves. Accounts registered against the old volume do not.)
#
# ── Frontend ───────────────────────────────────────────────────────────────────────
